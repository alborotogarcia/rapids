...
RUN cd /tmp; conda install -c conda-forge -c anaconda -y boto3 conda-build dash jupyter-dash bqplot plotly ipympl jupyter-lsp-python jedi-language-server python-lsp-server jupyterlab-lsp perspective jupyterlab-git turbodbc selenium minio mlflow psycopg2 voila lxml bs4 wget; conda install -y -c esri tensorflow-gpu; conda update --all; conda install -c conda-forge -y libthrift==0.14.1; git clone https://github.com/alborotogarcia/conda-spektral; cd conda-spektral/conda-recipes; conda build . ;conda clean -tipy;
